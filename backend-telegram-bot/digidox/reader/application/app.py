...
def calc_distance(line1, line2):
    line1_segs = np.array(
        list(
            zip(
                np.linspace(line1[0][0], line2[1][0], 100, dtype=int),
    ...
    line2_segs = np.array(
        list(
            zip(
                np.linspace(line2[0][0], line2[1][0], 100, dtype=int),
...
def process_passport(image, ocr_result, mrz_items):
    ...
    for line in ocr_result:
        ...
        with open("./reader/application/utils/fields_dict.txt", "r") as fp:
            for field in fp.readlines():
                field = field.rstrip("\n")
                if (
                    Levenshtein.distance(field, word) <= 3
                    and field not in keys_dict.keys()
                ):
                    keys_dict[field] = {"coords": coords, "center": word_center}
                elif (
                    Levenshtein.distance(field, word) > 3
                    and word not in values_dict.keys()
                    and field not in keys_dict.keys()
                ):
    ...
    for key_word, key_items in keys_dict.items():
        for value_word, value_items in new_values_dict.items():
            ...
            center_line1 = [
                [key_items["coords"][0][0], key_items["center"][1]],
                key_items["center"],
            ]
            ...
            if (act_dist > 0 and act_dist <= req_dist and act_dist != 13) and (
                avg_direction >= 0 and avg_direction <= 100
            ):
    ...
    if "KIM TOMONIDAN BERILGAN" in key_value_dict.keys():
        with open("./reader/application/utils/iib.txt", "r") as fp:
            for iib in fp.readlines():
                iib = iib.rstrip("\n")
                if (
                    Levenshtein.distance(
                        iib, " ".join(key_value_dict["KIM TOMONIDAN BERILGAN"])
                    )
                    <= 3
                ):
    ...
    for date in [d[:2] + "." + d[2:4] + "." + d[4:] for d in dates]:
        try:
            ...
            if int(real_date) > int(mrz_items["DATE OF BIRTH"].split(".")[2]) and int(
                real_date
            ) < int(mrz_items["DATE OF EXPIRY"].split(".")[2]):
                if date not in [
                    mrz_items["DATE OF BIRTH"],
                    mrz_items["DATE OF EXPIRY"],
                ]:
